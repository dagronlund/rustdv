...
Every `#[rustdv::test]` you have typed since Chapter 15 has been an IOU. This chapter pays it. Part IV is about to lean on two pieces of compile-time machinery — the test attribute and `#[derive(Component)]` — and the Python book's readers deserve to know exactly what replaced the decorators and metaclasses they left behind. That is a language question with a twist, because Python's tools ran at *import time* with the full power of a running program, and Rust has no import time at all.

> **In Python we...** wrapped functions in decorators — `@cocotb.test()` received our coroutine as an argument, registered it in a global list, and handed it back — and pyuvm went further: a *metaclass* registered every component class with the factory as a side effect of the `class` statement itself. Code ran when modules loaded, and frameworks used that moment to organize our programs for us.
...
That closes Part III, one chapter long on purpose. You now know how tests are found, how hierarchies will be walked, and why neither needs a global registry. Part IV can finally ask the Python book's biggest question in its new language: what is the UVM *for*, and what does it look like when the compiler is on the methodology's side? Chapter 22: Why UVM?
