...
| 21 | `ch21-macros` | Listings are bins under `src/bin/`. No simulator test — it is a macro demonstration, and it is the one crate permanently outside the chapter runs. Test and component registration use `linkme` distributed slices; the procedural macro implementation uses `syn` and `quote`, not raw token walking or hand-written section bounds. **The attribute is bare `#[component]` now** (cross-cutting note above); this is the chapter that teaches `#[derive(Component)]`, so any passage explaining what the argument selects has lost its subject. |
