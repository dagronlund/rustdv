...
## Part II onward — chapters 15–41: rustdv
...
| 30 | `ch30-variation-point-testbench-5.0` | 5 listings. TB 5.0. The generic `AluEnv<T>` is gone: a variation point chosen at *run* time is a factory `create_comp()` slot swapped by a type override, not a type parameter. A generic base survives only for variation chosen at *compile* time. The reason is concrete — a factory override cannot reach a type parameter. |
...
| 37 | `ch37-repair-desk-testbench-7.1` | 4 listings. TB 7.1 — **the repair desk, not Fibonacci.** (The chapter titles in `SUMMARY.md` still read the old way; fixing them is part of your job.) A clock-only device that does nothing but wait: `try_next_item`, random per-transaction latency, answers coming back out of order, matched by ticket. Teaching the mechanism on a device that cannot confuse it comes first *because the TinyALU cannot show it* — the TinyALU runs one operation at a time, so nothing ever returns out of order. |
...
| 40 | `rustdv/tinyalu_tb` | The same crate as the Interlude, now with everything taught. Where the Interlude showed the shape, this chapter walks it: why the BFM is in the ConfigDb rather than a singleton, why the components take no constructor arguments, why the scoreboard holds its own storage, and why two tests need no new components between them. **Do not cite `tinyalu_tb/src/lib.rs`** — the crate root is `tinyalu_tb.rs`, and no file in this project is named `lib.rs`. Worth one honest paragraph: this testbench drives the clock, because `sim/hdl/tinyalu.sv` is the bare DUT, while the book's copy of the design self-clocks and its chapters therefore do not. |
| 41 | *(none)* | No listings. The missing-pieces inventory. **ASK RAY** for the current list before writing it — the framework changed underneath this chapter, and an inventory that names the wrong gaps is worse than none. Two things that belong in it and are settled: rustdv is single-threaded by choice (one thread, no locks, which is what makes `Rc`/`RefCell` cheap and matches a simulator that is single-threaded anyway), and a transaction-level backend for emulation is out of scope — the BFM is the portability line, so swapping what is below it is a backend change rather than a framework change. |
...
subscriber declares two `SubscribePort`s and two `WriteSink` impls, so the
